...
- **Servicios (contrato):**
  ...
  - `marcar_recolectado(pedido, actor, paquetes=None)` — manifiesto: `paquetes` = cajas que suben a ESTE camión (None = todas las pendientes; pedido empacado entero sale completo). Cada caja pasa a `Paquete.DESPACHADO`; el kardex despacha las líneas de esas cajas la primera vez que salen (un kit arrastra a sus hijas); con cajas pendientes el pedido queda `PARCIALMENTE_DESPACHADO` (→ `RECOLECTADO` al salir la última). Plantilla B (mensajeria.enviar_en_camino) SOLO aquí y una sola vez por pedido (su primer manifiesto); fulfillment en Shopify cuando el pedido completo está fuera. El poller mueve el pedido por el CONJUNTO de guías (`envios.services._estado_por_guias`): ENTREGADO solo con todas las activas entregadas, RETORNADO solo con todas regresadas; nunca mientras queden cajas en bodega.
